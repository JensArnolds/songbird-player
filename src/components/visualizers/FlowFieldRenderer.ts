...
export class FlowFieldRenderer {
  ...
  private renderSpectralEcho(
    ...
    const stride = lowDetail ? (audioIntensity < 0.5 ? 3 : 2) : audioIntensity < 0.55 ? 2 : 1;
  ...
  private renderDemonicGate(
    ...
    gatePath.quadraticCurveTo(-halfGateWidth, -halfGateHeight, 0, -halfGateHeight);
  ...
  private renderShadowDance(
    ...
    if (lowDetail) {
      ctx.fillStyle = this.hsla(centerHue, 66, 18, 0.62 + audioIntensity * 0.25);
    } else {
  ...
  private renderEMField(
    ...
    for (let c = 0; c < chargeCount; c++) {
      ...
      const chargeHue = c === 0 ?
        this.fastMod360(fieldHue + 30) : this.fastMod360(fieldHue + 210);
